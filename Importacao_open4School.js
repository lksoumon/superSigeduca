    ...
    for (let i = 0; i < tabelas.length; i++) {
        ...
        if(tabelas[i].id.includes('GriddetalhesContainer_')){
            ...
            for (let j = 0; j < tabelas[i].getElementsByTagName('tr').length; j++) {
                //console.log(tabelas[i].getElementsByTagName('tr')[j].getElementsByTagName('span')[0].innerText.trim());
                if(j!=0){
                    temp.push(matriz);
...
function coletaDados() {
    var temp;
    var cabecalho = ["turma","codigo","Nº INEP","Nome do aluno","CPF do Aluno","RG do aluno","Órgão Expedidor","Sexo do Aluno","Data de Nascimento","Naturalidade","UF","Filiação 1","filiação 2",
                     "nome do responsável 1","CPF do responsável 1","DDD Residencial","Tel Res Resp 1","DDD Celular","Tel Celular Resp 1","DDD Comercial","Tel Comercial Resp 1","DDD Contato","Tel Contato Resp 1","E-mail Resp 1",
                      ...
                      'Endereço Rua','Número','Complemento','Bairro','Cidade','UF','CEP','UC (Distribuidora)','Nº UC'
                     ];
